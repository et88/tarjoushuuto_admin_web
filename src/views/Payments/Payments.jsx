...
const dimension_detail = () =>{
  ...
  React.useEffect(() => {
    ...
    const axios_get_api = (status,index=0,onLoad) => {
        ...
        let full_api = exportValue.host + '/' + exportValue.version + '/' + exportValue.api + `/payment_transaction_list`;
        let sendData = {
            find:"",
            status: status,
            indexValue: index,
  ...
  return (
    <div>
       ...
       <div class="modal-body">
           <table class="table">
               <tbody>
                 ...
                 <tr>
                   <td class="fw-700 text-dark">Shipment ID </td>
                   <td>{modals.shipment_id}</td>
                 </tr>
                 <tr>
                 ...
                 <tr>
                   <td class="fw-700 text-dark">Send from email</td>
                   <td>{modals.payer_email}</td>
                 </tr>
                 <tr>
                 ...
                 <tr>
                   ...
                   <td>{modals.currency_code}{modals.remaining_amount}</td>
                 </tr>
                 <tr>
       ...
       <button type="button" class="btn btn-link text-gray-600 ms-auto" onClick={()=>setModals({...modals,show:false})}>Close</button>
        ...
        <div className='row'>
            <div className='col-3'><Left_panel value={7}/></div>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
                <div class="d-block mb-4 mb-md-0">
                    ...
                    <h2 class="h4">Shipment Booking Payments</h2>
                </div>
            ...
            <section>
                <ul class="nav nav-tabs justify-content-end">
                    <li class="nav-item">
                      <a className={(shipmentstate.status == null) ? 'nav-link active' : 'nav-link'} onClick={() => shipmentFilter(null)}>All</a>
                    </li>
                    <li class="nav-item">
                      <a className={(shipmentstate.status == 1) ? 'nav-link active' : 'nav-link'} onClick={() => shipmentFilter(1)}>Success</a>
                    </li>
                    <li class="nav-item">
                      <a className={(shipmentstate.status == 2) ? 'nav-link active' : 'nav-link'} onClick={() => shipmentFilter(2)}>Pending</a>
                    </li>
                    {/* <li class="nav-item">
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="border-gray-200">Admin Fees</th>						
                            <th class="border-gray-200">Transporter Advance</th>
                            <th class="border-gray-200">Service Charge</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a onClick={()=>setModals({...modals,show:true,transaction_id:sub.transaction_id,shipment_id:sub.shipment_id,created:sub.created,date:sub.date,merchant_id:sub.merchant_id,full_name:sub.full_name,payer_email:sub.payer_email,payer_id:sub.payer_id,paid_amount:sub.paid_amount,currency_code:sub.currency_code,t_uid:sub.t_uid,payment_method:sub.payment_method,status:sub.status,service_charge:sub.service_charge,remaining_amount:sub.remaining_amount,transporter_advance:sub.transporter_advance,admin_advance:sub.admin_advance,invoice_id:sub.invoice_id})}  class="fw-700 text-info">{sub.transaction_id}</a>
                            </td>
                            <td>
                            <span class="fw-bold text-danger">
                                ...
                                <small>
                                {sub.currency_code}{sub.admin_advance} + {sub.currency_code}{sub.transporter_advance}+ {sub.currency_code}{sub.service_charge}
                                </small>
                            </td>
                            <td>
                                <span class="fw-bold">{sub.payment_method} </span><br />
                                     <small>{sub.payer_id}</small>
                            </td>
                            <td>
                            ...
                            {sub.currency_code}{sub.transporter_advance}
                               
                            </td>                        
                            <td>
                            {sub.currency_code}{sub.service_charge}
                            </td>
